...
#   MS_EXP=questa_teacher_b200 bash scripts/launch.sh teacher 30 [steps_per_cycle]
# Env: same as run_arm.sh (MS_ROOT, MS_MODEL, MS_CKPTS, MS_DATA, MS_WANDB=1 ..., MS_START_CYCLE / MS_SKIP_PREPARE for resume).
set -eu
ARM=${1:?usage: launch.sh adaptive|static|teacher <n_cycles> [steps_per_cycle]}; CYCLES=${2:-30}
K=${3:-${MS_K:-10}}          # steps between two controller/Teacher decisions
ROOT=${MS_ROOT:?set MS_ROOT}; WORK=${MS_WORK:-$ROOT/runs/$ARM}
TAG=$(date +%Y%m%d_%H%M%S)
mkdir -p $WORK/logs
STDOUT=$WORK/logs/${ARM}_${TAG}.stdout.log
MS_RUN_TAG=$TAG setsid nohup bash $ROOT/scripts/run_arm.sh $ARM $CYCLES $K > $STDOUT 2>&1 < /dev/null &
PID=$!
